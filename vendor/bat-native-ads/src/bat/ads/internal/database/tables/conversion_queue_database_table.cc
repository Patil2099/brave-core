...
void ConversionQueue::MigrateToV11(DBTransaction* transaction) {
  ...
  const std::string temp_table_name = get_table_name() + "_temp";
  ...
  // Migrate columns to temporary table
  ...
  util::Migrate(transaction, get_table_name(), temp_table_name, columns, true);
